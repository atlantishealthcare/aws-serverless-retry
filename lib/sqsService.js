...
SQSService.prototype.processMessages = function (queueName, maxNumberOfMessagesToRead, readConfigFromMessage, sqsConfig) {
    ...
    if (readConfigFromMessage === false) {
        if (!sqsConfig.retryTopicName) sqsConfig.retryTopicName = "retry-topic";
        if (!sqsConfig.failureTopicName) sqsConfig.failureTopicName = "failure-topic";
        if (!sqsConfig.maxRetryAttempts) sqsConfig.maxRetryAttempts = 1;
    }
        ...
        .then(response => {
            if (response && response.Messages) {
                ...
                let sendAndDeleteMessage = response.Messages.map(message => {
                    ...
                    if (parsedMessage) {
                        let isValidASRConfig = Util.validateASRConfig(parsedMessage.asrConfig);
                        if (isValidASRConfig) {
                            let destinationTopicName = readConfigFromMessage ? parsedMessage.asrConfig.retryTopicName : sqsConfig.retryTopicName;
                            let maxRetryAttempts = readConfigFromMessage ? parsedMessage.asrConfig.maxRetryAttempts : sqsConfig.maxRetryAttempts;
                            if (parsedMessage.retryCount > maxRetryAttempts) {
                                destinationTopicName = readConfigFromMessage ? parsedMessage.asrConfig.failureTopicName : sqsConfig.failureTopicName;
                            }
