...
SQSService.prototype.processMessages = function (queueName, maxNumberOfMessagesToRead, readConfigFromMessage, sqsConfig) {
    ...
    if (readConfigFromMessage === false) {
        ...
        if (!sqsConfig.retryTopicName) {
            Util.log("ERROR", "retryTopicName missing from SQSConfig");
            return Promise.reject({message: "retryTopicName missing from SQSConfig"});
        }
        ...
        .then(response => {
            if (response && response.Messages) {
                ...
                let sendAndDeleteMessage = response.Messages.map(message => {
                    ...
                    if (parsedMessage) {
                        let asrConfig = readConfigFromMessage ? parsedMessage.asrConfig : sqsConfig;
                        let isValidASRConfig = Util.validateASRConfig(asrConfig);
                        if (isValidASRConfig) {
                            let destinationTopicName = parsedMessage.retryCount > asrConfig.maxRetryAttempts ? asrConfig.failureTopicName : asrConfig.retryTopicName;
